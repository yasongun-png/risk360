...
// ---- Eğitim Katılım Formu (İmza Listesi) ----
...
// da imzalatabileyim" — kayıt oluşturmaz, sadece boş imza haneli, çok
// sayfaya bölünebilen bir PDF üretir (bkz. ui.js egitimImzaListesiPdfOlustur
// çağrısı).

const _EGT_IMZA_SATIR_SAYFA_BASI = 22;

function _egitimImzaListesiStilHtml(kokSelector) {
  return `
    <style>
...
function _egitimImzaListesiSayfaHtml(egitimTuruAdi, katilimcilar, firma, baslangicNo, sayfaNo, toplamSayfa) {
  ...
  const satirlar = katilimcilar.map((p, i) => `
    <tr>
      ...
      <td style="width:20%;">${_sertKacir(p.sicilNo)}</td>
      <td style="width:37%;">${_sertKacir(p.adSoyad)}</td>
      <td></td>
    </tr>
  `).join('');
  ...
  return `
    <section class="eil-sayfa">
      ...
      <div class="eil-bilgi">
        <span><b>Eğitim/Sertifika Türü:</b> ${_sertKacir(egitimTuruAdi)}</span>
...
async function egitimImzaListesiPdfOlustur(egitimTuruAdi, katilimcilar) {
  if (!Array.isArray(katilimcilar) || !katilimcilar.length) return;
  const firma = aktifFirmaGetir();

  const sayfalar = [];
  for (let i = 0; i < katilimcilar.length; i += _EGT_IMZA_SATIR_SAYFA_BASI) sayfalar.push(katilimcilar.slice(i, i + _EGT_IMZA_SATIR_SAYFA_BASI));

  const html = `<div id="egtImzaListesiPdf">${_egitimImzaListesiStilHtml('#egtImzaListesiPdf')}${
    sayfalar.map((s, i) => _egitimImzaListesiSayfaHtml(egitimTuruAdi, s, firma, i * _EGT_IMZA_SATIR_SAYFA_BASI + 1, i + 1, sayfalar.length)).join('')
  }</div>`;
  ...
  for (let i = 0; i < sayfaElemanlari.length; i++) {
    const canvas = await _egitimSayfaCanvasaCevir(sayfaElemanlari[i]);
