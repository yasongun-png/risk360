...
function raporListesiYazdir(baslik, altBilgi, kolonlar, satirlar) {
  ...
  const govde = satirlar.map(satir => `<tr>${kolonlar.map(k => `<td>${_raporKacir(satir[k.anahtar] ?? '-')}</td>`).join('')}</tr>`).join('');
...
function raporKartiYazdir(baslik, altBilgi, alanlar, fotograflar) {
  const satirlar = alanlar.map(a => `<tr><td style="font-weight:700; width:220px;">${_raporKacir(a.etiket)}</td><td>${_raporKacir(a.deger ?? '-')}</td></tr>`).join('');
